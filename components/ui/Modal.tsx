...
export default function Modal({ open, onClose, children, title }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      <div className="absolute inset-0 bg-charcoal/50 backdrop-blur-sm" onClick={onClose ?? undefined} />
      <div className="relative w-full max-w-md rounded-2xl bg-warm-white p-6 shadow-xl">
        <div className="mb-4 flex items-center justify-between">
          ...
          {onClose && <button onClick={onClose} className="ml-auto rounded-full p-1 text-muted hover:bg-cream hover:text-charcoal">✕</button>}
        </div>
        {children}
      </div>
    </div>
